...
engine = create_engine('sqlite:///camcatalog.db',connect_args={'check_same_thread': False}, echo=True)
...
def createUser(login_session):
    ...
    session.add(newUser)
    session.commit()
    user = session.query(UserData).filter_by(email=login_session['email']).one()
    #session.close()
    return user.id
...
#JSON END API's
#returns all models of particular brand as JSON object and Model as JSON object.
...
def BrandModelsJSON(brand_id):
    ...
    session.close()
    return jsonify(brand=[i.serialize for i in brand],model=[i.serialize for i in model])
...
def showBrands():
    ...
    return render_template('showbrands.html',brands=brands,cur_user=login_session)
    #return render_template('base.html',brands=brands,cur_user=login_session)
...
def editBrand(brand_id):
    ...
    if 'username' in login_session:
        if login_session['user_id'] == editBrd.user_id:
            ...
            #Print message
            flash("Permissions deniend !,U Don't Edit Brand")
            return redirect('/')
    else:
        #Print message
        flash("Login to Proceed !")
...
def deleteBrand(brand_id):
    ...
    if 'username' in login_session:
        if login_session['user_id'] == deleteBrd.user_id:
            ...
            #flash message
            flash("Permissions deniend !,U Don't Delete Brand")
            return redirect('/')
    else:
        #flash message....
        flash("Login to Proceed !")
...
def showModel(brand_id):
        ...
        return render_template('showModelsforuser.html',brand=brand,model=model,cur_user=login_session)
...
def newModel(brand_id):
    if 'username' in login_session:
        ...
        if login_session['user_id'] == brand.user_id:
            ...
            #flash message
            flash("Permissions deniend !,U Don't Create Model")
            return redirect('/')
    else:
        #flash message
        flash("Login to Proceed !")
    ...
    if 'username' in login_session:
        ...
        if login_session['user_id'] == brand.user_id:
            if request.method == 'POST':
                ...
                return render_template(
                    'EModel.html',brand_id=brand_id,model_id=model_id,edit=editMod,
                    cur_user=login_session)
                '''return render_template(
                    'editModel.html', brand_id=brand_id,model_id=model_id,
                    model=editMod, cur_user=login_session)'''
        else:
            #flash message
            flash("Permissions deniend !,U Don't Edit Model")
            return redirect('/')
    else:
        #flash message
        flash("Login to Proceed !")
    ...
    if 'username' in login_session:
        ...
        if login_session['user_id'] == deleteMod.user_id:
            if request.method == 'POST':
                ...
                #flash message
                # flash( " Model, Successfully deleted.")
                #return redirect(url_for('showModels', brand_id=brand_id,cur_user=login_session))
            else:
                return render_template(
            ...
            #flash message
            flash("Permissions deniend !,U Don't Delete Model")
            return redirect('/')
    else:
        #flash message
        flash("Login to Proceed !")
